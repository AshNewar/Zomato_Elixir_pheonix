defmodule Zomato.ReviewsTest do
  use Zomato.DataCase
  ...
  describe "reviews" do
    ...
    import Zomato.ReviewsFixtures

    @invalid_attrs %{description: nil, rate: nil, likes: nil}
    ...
    test "create_review/1 with valid data creates a review" do
      valid_attrs = %{description: "some description", rate: 42, likes: 42}

      assert {:ok, %Review{} = review} = Reviews.create_review(valid_attrs)
      assert review.description == "some description"
      assert review.rate == 42
      assert review.likes == 42
    end
